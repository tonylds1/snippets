...
## ⭐ Prompt pronto — revisão do índice (colar inteiro, de uma vez)

Preâmbulo de contenção + os 10 itens, já montados. É este que se usa na prática; a ETAPA 4
logo abaixo é a versão avulsa.
...
⚠️ A última frase existe porque numa execução real o modelo acrescentou uma seção de
`backfill` no `application.yml` — justamente o que o item 7 proíbe.
